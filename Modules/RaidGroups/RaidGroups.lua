...
function RaidGroups:NormalizeName(name)
...
function RaidGroups:ValidateAndNormalizePresetString(text)
    ...
    for i, chunk in ipairs(parts) do
        ...
        for name in strgmatch(namesStr or "", "([^,]+)") do
            ...
            if clean ~= "" then
                tinsert(names, self:NormalizeName(clean))
            end
        end
...
function RaidGroups:InvalidateRosterCache()
    self._classByName = nil
    E:SendMessage("ART_RAIDGROUPS_ROSTER_INVALIDATED")
end

function RaidGroups:InvalidateGuildCache()
    self._guildCache = nil
    self._guildCacheAt = 0
    self._classByName = nil
end

local function buildClassMap(self)
    ...
    if IsInGroup() or IsInRaid() then
        for i = 1, GetNumGroupMembers() do
            ...
            if name then
                ...
                map[self:NormalizeName(display)] = class
            end
        end
    end

    if self._guildCache then
        for _, entry in ipairs(self._guildCache) do
            local k = self:NormalizeName(entry.name)
            if not map[k] then
                map[k] = entry.class
            end
        end
...
function RaidGroups:RebuildGuildCache()
    ...
    self._guildCache = g
    self._guildCacheAt = GetTime()
    self._classByName = nil
end

function RaidGroups:EnsureGuildCache()
    if self._guildCache and (GetTime() - (self._guildCacheAt or 0)) < GUILD_CACHE_TTL then
        return
    end
    self:RebuildGuildCache()
end

function RaidGroups:GetClassForName(name)
    if not name or name == "" then
        return nil
    end
    self._classByName = self._classByName or buildClassMap(self)
    return self._classByName[self:NormalizeName(name)]
end
...
function RaidGroups:OnPlayerEnteringWorld()
...
function RaidGroups:OnGuildRosterUpdate()
    ...
    local hasData = self._guildCache and #self._guildCache > 0
    if hasData and (GetTime() - (self._guildCacheAt or 0)) < GUILD_REBUILD_THROTTLE then
        return
    end
    self:RebuildGuildCache()
    E:SendMessage("ART_RAIDGROUPS_ROSTER_INVALIDATED")
...
function RaidGroups:OnPlayerGuildUpdate()
    self:InvalidateGuildCache()
    E:SendMessage("ART_RAIDGROUPS_ROSTER_INVALIDATED")
end
...
function RaidGroups:OnRegenEnabled()
    if self._pendingUIRefresh then
        self._pendingUIRefresh = false
        E:SendMessage("ART_RAIDGROUPS_ROSTER_INVALIDATED")
    end
...
function RaidGroups:OnRosterUpdate()
    self._classByName = nil
    if InCombatLockdown() then
        self._pendingUIRefresh = true
        return
    end
    -- Editor reacts via ART_RAIDGROUPS_ROSTER_INVALIDATED 
    E:SendMessage("ART_RAIDGROUPS_ROSTER_INVALIDATED")
    if self._needGroup then
        if self._processTimer then
            self._processTimer:Cancel()
