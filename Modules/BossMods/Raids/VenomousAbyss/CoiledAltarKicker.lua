...
function CoiledAltarKicker:UpdatePreviewDisplay()
    ...
    local playerToken = UnitName("player") or "Player"
    local displayName, classFile = self:GetKickDisplayInfo(playerToken)
    ...
    self.frames.kickBox:ClearAllPoints()
    self.frames.kickBox:SetPoint("CENTER", self.frames.anchor, "CENTER", 0, 0)
    self.frames.kickBox:Show()

    if self.db.nextText.enabled then
        self.frames.nextTextAnchor:Show()
    else
        self.frames.nextTextAnchor:Hide()
    end
end
...
function CoiledAltarKicker:PlayPersonalAudioForCount(count)
    if self.editMode or not self.interruptActive or not self.assignedBossUnit then
        return
    end
...
function CoiledAltarKicker:HandleCastStart(unit)
    ...
    then
        ...
        if self.assignedBossUnit then
            self:PlayPersonalAudioForCount(
                self.castCountsByUnit[self.assignedBossUnit] or 1
            )
        end
    end
end
