...
local function isDifficultyEnabled(settings)
...
AbilityAlerts.entombedCastCounts = {}
AbilityAlerts.entombedLastCast = {}
AbilityAlerts.entombedAssignment = nil
AbilityAlerts.entombedAssignmentResolved = false
...
local function ensureBarEndAudioSettings(settings)
    if type(settings) ~= "table" then
        return nil
    end
    ...
    local audio = settings.barEndAudio

    if audio.enabled == nil then
        audio.enabled = false
    end

    audio.mode = audio.mode == "sound" and "sound" or "tts"
    audio.sound = audio.sound or "None"
    audio.channel = audio.channel or "Master"
    audio.ttsText = audio.ttsText or "{spell}"
    audio.voiceID = tonumber(audio.voiceID) or 0
...
local function findPlayerPositionInHashTag(
    ready,
    context,
    tag,
    maxPosition
...
local function getAbilityAssignment(ability)
    ...
    elseif spellID == RAVENOUS_FEAST_SPELL_ID then
        local hits = findPlayerHitsInTags(
            ready,
            context,
            "TFFeast",
            3
...
local function getRavenousFeastAssignedHits()
    local ready, context = getReadyAssignmentContext()
...
local function getMightyThudAssignedHits()
    local ready, context = getReadyAssignmentContext()
...
local function getEntombedRaidGroupAssignment()
    ...
    if subgroup == 3 or subgroup == 4 then
        return "red"
    end
...
local function getEntombedNoteAssignment()
    local ready, context = getReadyAssignmentContext()
...
local function getEntombedAssignment()
    ...
    if not issecretvalue(difficultyID) and tonumber(difficultyID) == 16 then
        return getEntombedRaidGroupAssignment()
    end

    return getEntombedNoteAssignment()
end
...
function AbilityAlerts:EnsureCustomBarDefaults()
    ...
    local enableAllBarsMigration =
        self.db.enableAllVenomousBarsMigration ~= 1
    ...
    for spellID, ability in pairs(self.abilitiesBySpellID) do
        do
            ...
            if enableAllBarsMigration then
                settings.bar.enabled = true
            elseif settings.bar.enabled == nil then
                settings.bar.enabled = true
            end

            if settings.venomousBarPresetVersion ~= 1 then
                ...
                settings.venomousBarPresetVersion = 1
            end
...
function AbilityAlerts:GetAbilitySettings(spellID)
    ...
    ensureBarEndAudioSettings(settings)
    return settings
end

local function copyColor(value, fallback)
    value = type(value) == "table" and value or {}
    fallback = fallback or {0.20, 0.60, 1.00, 1.00}
...
local function copyAudioSettings(source, includeTiming)
    source = type(source) == "table" and source or {}
    local result = {
        enabled = source.enabled == true,
        mode = source.mode == "sound" and "sound" or "tts",
        sound = source.sound or "None",
        channel = source.channel or "Master",
        ttsText = source.ttsText or (includeTiming and "{spell} in {time}" or "{spell}"),
        voiceID = tonumber(source.voiceID) or 0
    }

    if includeTiming then
        result.secondsBefore = tonumber(source.secondsBefore) or 3
        result.delayBy = tonumber(source.delayBy) or 0
        result.countdown = source.countdown == true
    end

    return result
end

function AbilityAlerts:EnsureAlertTypeGroups()
    self.db.alertTypeGroups = type(self.db.alertTypeGroups) == "table"
        and self.db.alertTypeGroups
        or {}

    local container = self.db.alertTypeGroups
    container.groups = type(container.groups) == "table" and container.groups or {}
    container.nextID = math.max(1, tonumber(container.nextID) or 1)

    if container.normalizedVersion ~= 1 then
        for id, group in pairs(container.groups) do
            group.id = tonumber(group.id) or tonumber(id) or container.nextID
            group.name = tostring(group.name or ("Alert Group " .. group.id))
            group.abilities = type(group.abilities) == "table" and group.abilities or {}
            group.fillColor = copyColor(group.fillColor)
            group.audio = copyAudioSettings(group.audio, true)
            group.barEndAudio = copyAudioSettings(group.barEndAudio, false)
            group.unattached = group.unattached == true
            group.position = type(group.position) == "table" and group.position or {}
            group.position.point = normalizeAnchorPoint(group.position.point)
            group.position.x = tonumber(group.position.x) or -400
            group.position.y = tonumber(group.position.y) or 80
            group.growth = group.growth == "UP" and "UP" or "DOWN"
            group.spacing = tonumber(group.spacing) or 4
            container.nextID = math.max(container.nextID, group.id + 1)
        end

        container.normalizedVersion = 1
    end
...
function AbilityAlerts:GetAlertTypeGroups()
    return self:EnsureAlertTypeGroups().groups
end


function AbilityAlerts:CreateAlertTypeGroup(name)
    local container = self:EnsureAlertTypeGroups()
    local id = container.nextID
    container.nextID = id + 1

    container.groups[id] = {
        id = id,
        name = tostring(name or ("Alert Group " .. id)),
        abilities = {},
        fillColor = {0.20, 0.60, 1.00, 1.00},
        audio = copyAudioSettings(nil, true),
        barEndAudio = copyAudioSettings(nil, false),
        unattached = false,
        position = {point = "CENTER", x = -400, y = 80},
        growth = "DOWN",
        spacing = 4
    }

    return container.groups[id]
end


function AbilityAlerts:DeleteAlertTypeGroup(groupID)
    local groups = self:GetAlertTypeGroups()
    groups[tonumber(groupID) or groupID] = nil
    self:ApplyPositions()
end


function AbilityAlerts:GetAlertTypeGroupForAbility(spellID, requireUnattached)
    spellID = tonumber(spellID)

    for _, group in pairs(self:GetAlertTypeGroups()) do
        if group.abilities[spellID] or group.abilities[tostring(spellID)] then
            if not requireUnattached or group.unattached then
                return group
            end
        end
    end
...
function AbilityAlerts:SetAlertTypeGroupAbility(groupID, spellID, enabled)
    ...
    if enabled then
        for _, otherGroup in pairs(groups) do
            otherGroup.abilities[spellID] = nil
            otherGroup.abilities[tostring(spellID)] = nil
        end
        group.abilities[spellID] = true
        self:ApplyAlertTypeGroup(groupID)
    else
        group.abilities[spellID] = nil
        group.abilities[tostring(spellID)] = nil
    end
...
function AbilityAlerts:ApplyAlertTypeGroup(groupID)
    local group = self:GetAlertTypeGroups()[tonumber(groupID) or groupID]

    if not group then
        return
    end

    for spellID, selected in pairs(group.abilities) do
        if selected then
            local settings = self:GetAbilitySettings(spellID)

            if settings then
                settings.bar = type(settings.bar) == "table" and settings.bar or {}
                settings.bar.fillColor = copyColor(group.fillColor)
                settings.bar.individualFillColorInitialized = true
                settings.audio = copyAudioSettings(group.audio, true)
                settings.barEndAudio = copyAudioSettings(group.barEndAudio, false)
            end
        end
    end
...
function AbilityAlerts:StartBar(ability, settings, durationOverride, hitNumber)
    ...
    bar:SetMiddle(getAbilityAssignment(ability) or "")
...
function AbilityAlerts:ShowText(
    ...
    local assignment = getAbilityAssignment(ability)
...
function AbilityAlerts:PlayBarEndAudio(ability, settings)
    local audio = ensureBarEndAudioSettings(settings)

    if not settings.audio or not settings.audio.enabled
        or not audio or not audio.enabled
    then
        return
    end
    ...
    BossMods.Alerts:StopTTS()
    BossMods.Alerts:SpeakTTS({
        text = replaceVariables(audio.ttsText, ability, 0),
        voiceID = tonumber(audio.voiceID) or 0
    })
end

function AbilityAlerts:ScheduleBarEndAudio(ability, settings, delay, token)
    local audio = ensureBarEndAudioSettings(settings)
    ...
    self:Schedule(ability.spellID, delay, token, function()
        self:PlayBarEndAudio(ability, settings)
    end)
end
...
function AbilityAlerts:IsAbilityFeatureEnabled(ability)
    if BossMods and BossMods.IsFeatureEnabled
        and not BossMods:IsFeatureEnabled("VenomousAbyss" .. ability.bossKey)
    then
        return false
    end
...
function AbilityAlerts:ResetEntombedCastTracking()
...
function AbilityAlerts:ShouldSuppressEntombedCast(spellID)
    spellID = tonumber(spellID)
...
function AbilityAlerts:HandleStandardAbility(ability, duration)
    ...
    if not isDifficultyEnabled(settings) then
        return
    end
    ...
    if settings.bar and settings.bar.enabled then
        ...
        self:ScheduleBarEndAudio(
            ability,
            settings,
            adjustedDuration + delayBy,
            token
        )
    end
...
function AbilityAlerts:OnBigWigsStartBar(spellKey, bigWigsText, duration)
    ...
    local ability = self:GetAbility(spellID)
    local suppressEntombedCast =
        self:ShouldSuppressEntombedCast(spellID)

    if ability then
        if not suppressEntombedCast then
            self:HandleStandardAbility(ability, duration)
        end
    end

    if suppressEntombedCast then
        return
    end
    ...
    for _, triggeredAbility in ipairs(
        ...
        local difficultyEnabled = isDifficultyEnabled(settings)
...
function AbilityAlerts:ApplyPositions()
    ...
    local function sortedAttachedSpellIDs(
        ...
        for spellID in pairs(collection) do
            ...
            if isActive
                ...
                and not self:GetAlertTypeGroupForAbility(spellID, true)
            then
    ...
    for _, group in ipairs(orderedGroups) do
        ...
        for spellID, selected in pairs(group.abilities) do
            ...
            local bar = numericSpellID and self.bars[numericSpellID]

            if selected and bar and bar.IsRunning and bar:IsRunning() then
                groupBars[#groupBars + 1] = numericSpellID
            end
        end

        table.sort(groupBars, function(a, b)
            local abilityA = self:GetAbility(a)
            local abilityB = self:GetAbility(b)
            local bossA = abilityA and abilityA.bossOrder or 100
            local bossB = abilityB and abilityB.bossOrder or 100

            if bossA ~= bossB then
                return bossA < bossB
            end

            return (abilityA and abilityA.order or 100)
                < (abilityB and abilityB.order or 100)
        end)

        local position = group.position or {}
        local direction = group.growth == "UP" and 1 or -1
        local offset = 0

        for _, spellID in ipairs(groupBars) do
            local bar = self.bars[spellID]
            local point = normalizeAnchorPoint(position.point)
            ...
            bar.frame:SetPoint(
                ...
                UIParent,
                "CENTER",
                tonumber(position.x) or -400,
                (tonumber(position.y) or 80) + offset * direction
            )
            updateAnchorPointMarker(bar.frame, point)

            local appearance = self:GetBarAppearance(spellID)
            offset = offset
                + (tonumber(appearance and appearance.height) or 24)
                + (tonumber(group.spacing) or 4)
        end
    end
...
function AbilityAlerts:ENCOUNTER_END()
    self:ResetEntombedCastTracking()
    self:ResetAlerts()
end
